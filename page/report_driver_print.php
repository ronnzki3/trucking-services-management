...
<div class="main-content">
    <div class="content-wrapper">
        <div class="header-title">
            ...
            <h1 style="text-align:center;font-weight:400;">Trips per Driver <span class="prnt-prvw">- Print Preview</span></h1>
        </div>
        <hr />
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php
                                ...
                                // UNION ALL 
                                // SELECT ca_date as tripdate,driverid,'' as destinationid,'' as plateid,'' as driver,'' as destination,'' as areacode,'' as plate,'' as rateamount,'' as diesel,amount,remarks 
                                // FROM ca
                                // Where driverid=".$driverid." AND ca_date>='".$date1."' AND ca_date<='".$date2."'
                                // AND ca_date NOT IN (SELECT tripdate FROM vwdailytripall WHERE driverid=".$driverid." AND ca_date>='".$date1."' AND ca_date<='".$date2."') ORDER BY tripdate ASC");
                                
                                $res=$db->custom_select("SELECT tripdate,driverid,destinationid,plateid,driver,destination,areacode,plate,rateamount,diesel,
                                (SELECT SUM(amount) as amount FROM vwcasum WHERE ca_date>=tripdate AND ca_date<=tripdate) as amount 
                                FROM vwdailytripall
