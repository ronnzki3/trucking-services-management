        ...
        <div class="rpplate-container">  
                <div class="rpplate-wrapper">
                    <div class="container-srch">     
                        ...
                        <a href="report_truck_print.php?id=<?=$plateid?>&datefr=<?=$date1?>&dateto=<?=$date2?>&filter=<?=$filterid?>"> <img src="../icons/printpreview.png" style="width:25px;"> Print Preview</a>
                    </div>                 
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php
                                ...
                                $i=1;
                                $resy=$db->select('vwdailytripall','*',"plateid='".$plateid."' AND tripdate>='".$date1."' AND tripdate<='".$date2."' ORDER BY tripdate ASC");
                                while($rwy=mysqli_fetch_assoc($resy)){
                            ?>
                            <tr>
                                <td><?=$i++;?></td>
                                <td>
                                    <?=date('M d, Y',strtotime($rwy['tripdate']));?>
                                </td>
                                <td>
                                    <?=$rwy['destination'].' (<span class="areacodetbl">'.$rwy['areacode'].'</span>)';?>
                                </td>
                                <!-- <td>
                                    <?=$rwy['driver'];?>
                                </td> -->
                                <td style="text-align:right;padding-right:5px;">
                                    <?=number_format($rwy['destination_rate']);?>
                                </td>
                                <td>
                                    <?=number_format($rwy['totalexpenses']);?>
                                </td>
                            </tr>
                            <?php
                                }
                        ...
                        <tfoot>
                            <?php
                            $gtotal=0;
                            $res=$db->select('vwdailytripall','SUM(destination_rate) as totalrate',"plateid=".$plateid." AND tripdate>='".$date1."' AND tripdate<='".$date2."' ORDER BY tripdate ASC");
                            while($rww=mysqli_fetch_assoc($res)){
                                $gtotal=$rww['totalrate'];
                            }
                           ...
                           <tr style="background-color:white;color:black;">
                                <td colspan="6">
                                    ...
                                    <span style="font-size:1.2rem;font-weight:700;margin-left:10px;color:black;"><?=number_format($gtotal);?></span>
                                </td>
                            </tr>
                            <tr style="background-color:white;color:black;">
                                <td colspan="6">
                                    <a href="report_truck_expenses.php?id=<?=$plateid2?>&datefr=<?=$date1?>&dateto=<?=$date2?>&filter=7"> see details > </a>
                                    <span style="font-weight:500;">Expenses Total:</span>                                    
                                    <span style="font-size:1.2rem;font-weight:700;margin-left:10px;color:red;"><?=number_format($gtotalexpenses);?></span>
                                </td>
                            </tr>
                            <tr style="background-color:white;color:black;">
                                <td colspan="6">
                                    ...
                                    ?>
                                    <a href="report_truck_maintenance.php?id=<?=$plateid2?>&datefr=<?=$date1?>&dateto=<?=$date2?>&filter=7"> see details > </a>
                                    <?php
                                    }
                                    ?>
                                    
                                    <span style="font-weight:500;">Maintenance Expenses Total:</span>                                    
                                    <span style="font-size:1.2rem;font-weight:700;margin-left:10px;color:red;"><?=number_format($totalmaintenance);?></span>
                                </td>
                            </tr>
                            ...
                            <tr style="background-color:white;color:black;">
                                <td colspan="6">
                                    ...
                                    <span style="font-size:1.2rem;font-weight:700;margin-left:10px;color:blue;"><?=number_format($netpay);?></span>
                                </td>
                            </tr>
